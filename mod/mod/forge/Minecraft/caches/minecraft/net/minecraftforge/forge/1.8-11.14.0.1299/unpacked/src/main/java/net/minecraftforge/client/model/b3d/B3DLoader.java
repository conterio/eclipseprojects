...
public class B3DLoader implements ICustomModelLoader
{
    ...
    public static class Wrapper extends PartWrapper<Mesh> implements IModel
    {
        ...
        private static ImmutableMap<String, ResourceLocation> buildTextures(ResourceLocation location, List<Texture> textures)
        {
            ...
            for(Texture t : textures)
            {
                ...
                builder.put(t.getPath(), new ResourceLocation(location.func_110624_b(), path));
            }
    ...
    private static class BakedWrapper implements IFlexibleBakedModel, ISmartBlockModel, ISmartItemModel
    {
        ...
        public BakedWrapper(B3DLoader.Wrapper model, IModelState state, VertexFormat format, ImmutableMap<String, TextureAtlasSprite> textures)
        {
            ...
            buf = BufferUtils.createByteBuffer(VERTICES_IN_QUAD * format.func_177338_f());
        }
            ...
            if(quads == null)
            {
                ...
                for(Node<?> child : mesh.getNodes().values())
                {
                    if(child.getKind() instanceof Mesh)
                    {
                        ...
                        builder.addAll(new BakedWrapper(new B3DLoader.Wrapper(model.getLocation(), model.getTextureMap(), childMesh), state, format, textures).func_177550_a());
                    }
                }
                ...
                for(Face f : faces)
                {
                    ...
                    putVertexData(f.getV3(), sprite);
                    buf.flip();
                    int[] data = new int[VERTICES_IN_QUAD * format.func_177338_f() / BYTES_IN_INT];
                    buf.asIntBuffer().get(data);
                    builder.add(new BakedQuad(data, -1, EnumFacing.func_176737_a(f.getNormal().x, f.getNormal().y, f.getNormal().z)));
                }
                quads = builder.build();
            }
            return quads;
        ...
        private final void putVertexData(Vertex v, TextureAtlasSprite sprite)
        {
            ...
            for(VertexFormatElement e : (List<VertexFormatElement>)format.func_177343_g())
            {
                ...
                case UV:
                    ...
                    if(e.func_177369_e() < v.getTexCoords().length)
                    {
                        put(e,
            ...
            buf.position(oldPos + format.func_177338_f());
        }
