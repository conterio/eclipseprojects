    ...
    public static class BreakEvent extends BlockEvent
    {
        ...
        public BreakEvent(World world, BlockPos pos, IBlockState state, EntityPlayer player)
        {
            ...
            if (state == null || !ForgeHooks.canHarvestBlock(state.func_177230_c(), player, world, pos) || // Handle empty block or player unable to break block scenario
                (state.func_177230_c().canSilkHarvest(world, pos, world.func_180495_p(pos), player) && EnchantmentHelper.func_77502_d(player))) // If the block is being silk harvested, the exp dropped is 0
            {
                this.exp = 0;
            }
            else
            {
                ...
                this.exp = state.func_177230_c().getExpDrop(world, pos, bonusLevel);
            }
        }
